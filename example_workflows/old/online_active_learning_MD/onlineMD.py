...
def onlineMD(candidate_parameters, initial_parameters_file=None):
    ...
    for i in range(N_sim):
        ...
        sim_dir_list.append(
            f"{candidate_parameters[i]['Temp_K']}_K_{candidate_parameters[i]['Strain']}_lattice_{candidate_parameters[i]['Shear_Strain']}_shear"
        )
    ...
    edr = EnsembleDynamicsRunner(
        ...
        tasks_per_chore=input_params["total_procs"],
        cores_per_task=1,
        gpus_per_task=0,
...
def gather_data(dirs, params):
    ...
    for dir in dirs:
        if params["filename_pattern"] == "twsit_":
            data = pd.read_csv(
                f"{os.path.abspath(dir)}/{params['filename_pattern']}{params['Tstart']}",
                sep="\s+",
                converters={"twist-angle": lambda x: float(x.strip("[]"))},
            )  # Convert string lists to float
        if params["filename_pattern"] == "coverage_probability_":
            data = pd.read_csv(
                f"{os.path.abspath(dir)}/{params['filename_pattern']}{params['Tstart']}",
                sep="\s+",
                converters={"coverage_prob": lambda x: float(x.strip("[]"))},
            )

        for timestep in range(
            int(params["Tstart"] + params["Tstep"]),
            int(params["Tend"] + params["Tstep"]),
            int(params["Tstep"]),
        ):
            if params["filename_pattern"] == "twsit_":
                temp_df = pd.read_csv(
                    f"{os.path.abspath(dir)}/{params['filename_pattern']}{timestep}",
                    sep="\s+",
                    converters={"twist-angle": lambda x: float(x.strip("[]"))},
                )  # Convert string lists to float
            if params["filename_pattern"] == "coverage_probability_":
        ...
        if params["filename_pattern"] == "coverage_probability_":
            output_data["output"].append(np.mean(np.array(data["coverage_prob"])))
...
if __name__ == "__main__":
    ...
    grid = np.array(
        np.meshgrid(temperature_data, lattice_strain_data, shear_strain_data)
    ).T.reshape(-1, 3)
    params_dict = []
    ...
    for ic in range(partial_samples_init, partial_samples_final):
        params_dict.append(
            {"Temp_K": grid[ic, 0], "Strain": grid[ic, 1], "Shear_Strain": grid[ic, 2]}
        )

    onlineMD(
        candidate_parameters=params_dict, initial_parameters_file="input_paramters.json"
    )
