...
use system-specific examples for launch scripts, containers, scheduler flags, and science
dependency setup.
...
MatEnsemble workflows are centered around `Chores` which are delayed function calls that are
serialized and later called on compute resources that MatEnsemble has control over
