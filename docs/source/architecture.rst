...
:class:`~matensemble.pipeline.Pipeline`
    Builder for a directed acyclic graph (DAG) of :class:`~matensemble.job.Job` instances.
    Calling ``@pipe.job``-decorated functions records delayed work; :meth:`~matensemble.pipeline.Pipeline.exec`
...
:class:`~matensemble.job.Job`
    ...
    working directory, and (for Python jobs) import metadata and serialized arguments.
...
:class:`~matensemble.manager.FluxManager`
    ...
    blocked, and running job IDs, tracks free cores/GPUs from Flux, and drives the main scheduling loop.

:class:`~matensemble.fluxlet.Fluxlet`
    Thin wrapper that turns a :class:`~matensemble.job.Job` into a Flux ``JobspecV1`` and submits it
    through a :class:`flux.job.FluxExecutor`.

``matensemble.runtime_worker``
    A normal Python module launched **as the Flux job command** for Python-flavor jobs. It unpickles
    the job, imports your function, substitutes dependency results, runs the function, and writes
   ...
   └── matensemble_workflow-YYYYMMDD_HHMMSS/
       ...
       └── out/
           └── <job_id>/
               ├── stdout
               ├── stderr
               ├── job.pkl          # pickled Job (written at submit time)
               ├── job.json         # same metadata in JSON for debugging
               ├── result.pkl       # Python job return value (pickle)
...
**Python jobs** need importable callables. When a decorated function is registered, MatEnsemble sets
``PYTHONPATH`` in the job environment to the **parent directory of the workflow folder** (the
...
``importlib.import_module``. Keep your job modules on a normal package path under that root.
...
embedded in a Python job’s positional or keyword arguments (including nested tuples, lists, dicts,
and non-class dataclass instances). :meth:`matensemble.pipeline.Pipeline.exec` does **not** currently
accept dependency references; treat executable jobs as root tasks unless you wrap shell work inside
a Python job.

Before submit, MatEnsemble:

#. Builds a :class:`networkx.DiGraph` with an edge ``upstream → downstream`` for each dependency.
#. Verifies that every referenced job ID exists.
#. Rejects cycles (topological sort must succeed).

The :class:`~matensemble.manager.FluxManager` receives jobs in topological order, but **submission**
...
Each job declares :class:`~matensemble.model.Resources`:

* ``num_tasks`` — Flux task count for the job.
...
* The **total** allocation (all jobs must fit in the worst case—oversized jobs are marked invalid).
...
#. Drain the **ready** queue and submit every job that fits; defer the rest to the back of the queue.
...
If a job fails submission, raises in the Flux future wrapper, or returns a non-zero process exit code,
MatEnsemble records a failure and **cascades** to all transitive dependents so the workflow cannot deadlock.
Downstream jobs receive failure reason ``dependency_failed`` with an ``upstream`` job ID in the internal
failure list. Check per-job ``stderr`` for the detailed MatEnsemble annotations written by
