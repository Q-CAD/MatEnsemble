...
def _render_batch(**kwargs: Any) -> str:
    ...
    if key in HPC_SYSTEMS:
        return render_site_batch_script(
            ...
            job_name=slugify(kwargs["campaign"])[:32],
    ...
    if backend == "docker":
        return (
            "#!/usr/bin/env bash\n"
            f"#SBATCH -A {kwargs['account']}\n"
            f"#SBATCH -J {slugify(kwargs['campaign'])[:32]}\n"
            f"#SBATCH -t {kwargs['walltime']}\n"
            f"#SBATCH -N {int(kwargs['nodes'])}\n\n"
            "set -euo pipefail\n"
            f'docker run --rm -v "$PWD:$PWD" -w "$PWD" {container} '
            f"flux start --test-size={max(int(kwargs['tasks']), 1)} python workflow.py\n"
        )
    raise ValueError(f"unsupported batch system/backend combination: {key}/{backend}")
