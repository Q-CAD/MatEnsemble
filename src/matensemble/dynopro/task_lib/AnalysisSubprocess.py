...
def AnalysisSubprocess(comm, input_params):
    ...
    while stride < MAX_STRIDES:
        ...
        timestep = (
            stride * (nprocs - input_params["md_procs"])
            + (rank - input_params["md_procs"])
        ) * input_params["i_o_freq"]
        ...
        data = OvitoCalculators(
            lmp_snapshot=lmp_snapshot, species=input_params["species"], serialize=True
        )
        ...
        if "stream" in input_params.keys():
            ...
            rds = RedisService(
                host=input_params["stream"]["host"], port=input_params["stream"]["port"]
            )
        else:
            rds = None
        ...
        if "dxa_analysis" in input_params.keys():
            if input_params["dxa_analysis"]:
                data.calculate_dxa(
                    export_formats=input_params["dxa_output_formats"],
                    dxa_line_sep=input_params["dxa_line_sep"],
                    lattice="lattice",
                )

        if input_params["full_trajectory_dump"]:
            data.dump_trajectories(
                export_formats=input_params["trajectory_output_format"]
            )

        if "compute_twist" in input_params.keys():
            twist_angle = compute_twist.get_interlayer_twist(
                data,
                cutoff=input_params["compute_twist"]["cutoff"],
                reference_particle_type=input_params["compute_twist"][
                    "reference_particle_type"
                ],
            ...
            if "target_window" in input_params["compute_twist"].keys():
                ...
                prob = get_probability(
                    twist_angle,
                    target_window=input_params["compute_twist"]["target_window"],
                )
                with open(f"coverage_probability_{data.timestep}", "w") as file:
                    file.write(f"time-step coverage_prob min_twist max_twist\n")
                    file.write(
                        f"{data.timestep} {prob} {input_params['compute_twist']['target_window'][0]} {input_params['compute_twist']['target_window'][1]}"
                    )
        ...
        if "compute_rdf" in input_params.keys():
            try:
                rdf = correlations.compute_rdf(
                    data,
                    cutoff=input_params["compute_rdf"]["cutoff"],
                    number_of_bins=input_params["compute_rdf"]["number_of_bins"],
                    z_min=input_params["compute_rdf"]["z_min"],
                )
        ...
        if "compute_adf" in input_params.keys():
            try:
                adf = correlations.compute_adf(
                    data,
                    cutoff=input_params["compute_adf"]["cutoff"],
                    number_of_bins=input_params["compute_adf"]["number_of_bins"],
                    z_min=input_params["compute_adf"]["z_min"],
                )
        ...
        for analysis_name, analysis_func in registered_analyses.items():
            if analysis_name in input_params:
                if input_params[analysis_name].get("enabled"):
                    analysis_data = analysis_func(
                        data=data, params=input_params[analysis_name]
                    )
